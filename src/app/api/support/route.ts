  ...
  try {
    ...
    if (!subject || !message) {
      return NextResponse.json(
        { error: "validation", detail: "subject and message are required" },
        { status: 400 }
      );
    }
    ...
    await prisma.auditLog.create({
      data: {
        ...
        // Avoid runtime enum import; pass string and type it as the DB enum
        action: "SUPPORT_TICKET" as Prisma.AuditAction,
        target: "Ticket",
        targetId: null,
        meta: {
          subject,
    ...
    return NextResponse.json(
      { error: "support_failed", detail: err?.message || String(err) },
      { status: 500 }
    );
  }
}
